const test = `[1518-03-11 00:45] wakes up
...
let demo1 = `[1518-11-01 00:00] Guard #10 begins shift
...
const sleepiestGuardStrategy1 = (string) => {
string = string.replace(/\[1518-0/g ,'')
.replace(/\[1518-/g ,'')
.replace(/ 00:/g,'')
.replace(/ 23:[0-5][0-9]/g,'60')
.replace(/] /g,',')
.replace(/-/g,'')
...
let sleepiestGuard = '';
let sleepiestMinute = 99;

let array = string.split('\n').sort()
.map(event => event = event.split(','))
...
Object.keys(events).forEach(event => {
  if(events[event].includes('begins shift')){
    currentGuard = events[event].replace(/Guard #/,'').replace(/ begins shift/,'');
  }
 
  if(events[event].includes('falls asleep')) {
    startSleep = Number(event[event.length-2] + event[event.length-1]);
  }

  if(events[event].includes('wakes up')) {

  endSleep = Number(event[event.length-2] + event[event.length-1]);


    if (asleep[currentGuard]) {
      asleep[currentGuard] = asleep[currentGuard] + (endSleep - startSleep);
    } else {
      asleep[currentGuard] = 0 + (endSleep - startSleep);
    }

    for (x = startSleep; x < endSleep; x++) {
      if (!guards[currentGuard]) {
        guards[currentGuard] = {};
      }

      if (guards[currentGuard][x]) {
        ++guards[currentGuard][x];
      } else {
        guards[currentGuard][x] = 1;
      }
    }
...
for (minute in guards[sleepiestGuard]) {
  if (sleepiestMinute === 99) {
    sleepiestMinute = minute;
  }
  if (guards[sleepiestGuard][minute] > guards[sleepiestGuard][sleepiestMinute]) {
    sleepiestMinute = minute;
  }
}

return {sleepiestGuard, sleepiestMinute, solution: Number(sleepiestGuard) * Number(sleepiestMinute)};

}

const sleepiestGuardStrategy2 = (string) => {
...
for (guard in asleep) {
  for (minute in guards[guard]) {
    ...
    if (guards[guard][minute] > guards[sleepiestGuard][sleepiestMinute]) {
      sleepiestMinute = minute;
      sleepiestGuard = guard;
    }
  }
...
module.exports = {demo1, test, sleepiestGuardStrategy1, sleepiestGuardStrategy2}